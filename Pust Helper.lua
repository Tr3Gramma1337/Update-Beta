...
-- Скрипт
script_name('PustHelper')
script_version('Custom')
script_author('Katanniy') 
...
function main()
    ...
    if checkboxes.hello.v then
			if ints.active.v == 0 then
				msg('{7CFC00}By '..color..'Katanniy '..textcolor..'{7CFC00}Command '..color..'/phelper') 
			end
			if ints.active.v == 1 then
				msg('{7CFC00}By '..color..'Katanniy '..textcolor..'. {7CFC00}Command '..color..cfg.settings.cheatcode) 
			end
		end
    ...
    frames.window.v = false  --show window
	  ...
	  sampRegisterChatCommand('tr', function(num) 
	    if checkboxes.cmds.v then 
				sampSendChat('/trade '..num)
                        sampAddChatMessage('{FFFFFF}[{FF0000}TRADE{FFFFFF}] /tr [id]') 
			end
                end)
...
sampRegisterChatCommand('re', function(num)
    if checkboxes.cmds.v then
sampAddChatMessage('{ff0000}Recon to {00deff}'..num, -1)
        --printStringNow('~g~Recon to ~b~ player ~r~'..num, 3000)
    end
end)
sampRegisterChatCommand('azcoin', function(num)
    if checkboxes.cmds.v then
sampAddChatMessage('{00deff}AZ-Coins успешно выданы в количестве: {ff0000} '..num, -1)
    end
end)
sampRegisterChatCommand('additem', function()
    if checkboxes.cmds.v then
sampAddChatMessage("{FFFF00}Добавлено в инвентарь: Сертификат 'Pagani Zonda' Чтобы открыть инвентарь, нажмите 'I' или /invent", -1)
    end
end)
		sampRegisterChatCommand('urc', function(num)
			if checkboxes.cmds.v then  
                ...
                sampRegisterChatCommand('itemid', function()
                        if checkboxes.cmds.v then
                                msg('{ff0000}Ждун {ffffff}- {00deff}18092', -1)
                                msg('{ff0000}AZ-Coins {ffffff}- {00deff}9073', -1)
                                msg('{ff0000}Ящик Дрифта {ffffff}- {00deff}3308', -1)
                                msg('{ff0000}Мото-Ящик {ffffff}- {00deff}18449', -1)
                                msg('{ff0000}Бургер {ffffff}- {00deff}2191', -1)
                        end
                end)
                sampRegisterChatCommand('n', function(num)
                        if checkboxes.cmds.v then
                                sampSendChat('/number '..num)
                        end
                end)
                sampRegisterChatCommand('number', function()
                        if checkboxes.cmds.v then
                                sampAddChatMessage('{FFFFFF}[{FF0000}Number{FFFFFF}] /n [number]')
                        end
                end)
		sampRegisterChatCommand('lavka', function(num)
			if checkboxes.cmds.v then  
				sampSendChat('/findilavka '..num) 
			end
		end)
                sampRegisterChatCommand('bcar', function()
                        if checkboxes.cmds.v then
                                sampSendChat('/breakcar')
                        end
                end)
		...
		sampRegisterChatCommand('findilavka', function()
			if checkboxes.cmds.v then  
				msg('/lavka id')
			end
		end)
          sampRegisterChatCommand('yt', function() 
	    if checkboxes.cmds.v then 
				os.execute('start https://youtube.com/') 
			end
                end)
                sampRegisterChatCommand('shadowcommand', function()
                                      msg('{FF0000}/vk')
                                      msg('{FF0000}/autor')
                                end)
		...
		--end)
		sampRegisterChatCommand("sp", function() send('/setspawn') end)
		...
		sampRegisterChatCommand("fish", function()
			if checkboxes.job.v then
                ...
                sampRegisterChatCommand('forum', function()
                      os.execute('start https://forum.rodina-rp.com/')
                      printStringNow("~y~Forum Opened!", 5000)
                end)
          sampRegisterChatCommand('vk', function() 
	    if checkboxes.cmds.v then 
				os.execute('start https://vk.com/audios545411710?section=all') 
			end
                end)
	  sampRegisterChatCommand('i', function() 
	    if checkboxes.cmds.v then 
				sampSendChat('/invent') 
			end
                end)
           sampRegisterChatCommand('psave', function() 
             if checkboxes.cmds.v then 
	                     save()
             msg('Config Saved')
			end
	    ...
	    if checkboxes.cmds.v then 
				sampAddChatMessage('{FF0000}By Katanniy', -1) 
			end
                end)
                sampRegisterChatCommand('command', function()
                                sampAddChatMessage('{FF0000}-----------------------------------------', -1)
                                msg('/biz - /bizinfo [id]', -1)
                                msg('/car [id] - fixmycar', -1)
                                msg('/fh - /findihouse [id]', -1)
                                msg('/fbiz - /findibiz [id]', -1)
                                msg('/urc - /unrentcar', -1)
                                msg('/fin [id] [id biz] - /showbizinfo', -1)
                                msg('/sp - /setspawn', -1)
                                msg('/tr - /trade [id] {FF0000}(new) {0000FF}+f i x', -1)
                                msg('/i - /invent {FF0000}(new) {0000FF}+f i x', -1)
                                msg('/yt - start youtube {FF0000}(new)', -1)
                                msg('/cabinet - заходит в личный кабинет крмп {FF0000}(new)', -1)
                                msg('/lavka [id] - /findilavka [id] {FF0000}(new)', -1)
                                sampAddChatMessage('{FF0000}-----------------------------------------', -1)
                            end)
		sampRegisterChatCommand('cabinet', function()
			if checkboxes.cmds.v then  
				os.execute('start https://rodina-rp.com/profile')
			end
		end)
		...
		sampRegisterChatCommand('Pustmusic', function()
			if checkboxes.osplayer.v then 
				frames.musicmenu.v = not frames.musicmenu.v 
			else
				msg('Сначала включите Pust Music в главном меню.')
			end
		end)
...
while true do
    wait(0)
        ...
        if wasKeyPressed(VK_XBUTTON1) and not sampIsCursorActive() then       --бинд на боковую кнопку мыши (первую)
                        sampSendChat('/anims 35')
                        end
   ...
   --     inicfg.save(cfg, 'OSHelper.ini')
    ...
    -- hotkeys
        if not sampIsCursorActive() then
			...
			if ztimer == 0 then
				ztimer = ztimer - 1
				msg('Зетка слетела, можно съебаться с игры.')
				wait(1000)
			end
		end
	end -- cancel
...
function imgui.OnDrawFrame()
    ...
    if frames.window.v then
        ...
        imgui.Begin('Pust Helper '..thisScript().version, frames.window, imgui.WindowFlags.NoResize)
	        imgui.BeginChild("left", imgui.ImVec2(150, 290), true)
				...
				elseif imgui.Selectable(fa.ICON_FA_INFO_CIRCLE..u8' Информация', menu == 7) then menu = 7
                                elseif imgui.Selectable(fa.ICON_FA_COG..u8' TEST', menu == 10) then menu = 10
                                elseif imgui.Selectable(fa.ICON_FA_LINK..u8' Ссылки', menu == 11) then menu = 11
                                elseif imgui.Selectable(fa.ICON_FA_KEYBOARD..u8' code', menu == 12) then menu = 12
                                elseif imgui.Selectable(fa.ICON_FA_PAPER_PLANE..u8 ' Telegram', menu == 13) then menu = 13
                                elseif imgui.Selectable(fa.ICON_FA_LANGUAGE..u8' Язык', menu == 14) then menu = 14
                                elseif imgui.Selectable(fa.ICON_FA_MICROCHIP..u8' Цены', menu == 15) then menu = 15
				end
			...
			if menu == 7 then
        		...
        		imgui.PopFont()
				imgui.Separator()
				imgui.Text(u8'Spizdil')
				imgui.Text('')
				imgui.Text(u8'Разработчик:') imgui.SameLine() imgui.Link('https://vk.com/bushidojoo', 'Pustota')
				imgui.Text(u8'По поводу бага') imgui.SameLine() imgui.Link('https://i.ytimg.com/vi/WZ1hiSRmqiY/hqdefault.jpg', u8'Пиши сюда!')
			end
			if menu == 8 then
				imgui.PushFont(fontsize)
				...
				if checkboxes.fisheye.v then
					...
					if imgui.SliderInt('##FOV', sliders.fov, 1, 100) then cfg.settings.fov = sliders.fov.v end
                        ...
                        if menu == 10 then
				imgui.PushFont(fontsize)
        			imgui.CenterText(u8'TEST')
        		imgui.PopFont()
				imgui.Separator()
				imgui.Text(u8'')
                                imgui.SetCursorPosY(30)
                            if imgui.Button(u8'Сокращенные команды', imgui.ImVec2(150,20)) then
                                sampAddChatMessage('{FF0000}-----------------------------------------', -1)
                                msg('/biz - /bizinfo [id]', -1)
                                msg('/car [id] - fixmycar', -1)
                                msg('/fh - /findihouse [id]', -1)
                                ...
                                msg('/i - /invent {FF0000}(new) {0000FF}+f i x', -1)
                                msg('/yt - start youtube {FF0000}(new)', -1)
                                msg('/cabinet - открывает личный кабинет крмп {FF0000}(new)', -1)
                                sampAddChatMessage('{FF0000}-----------------------------------------', -1)
                                --os.execute("start https://i.ytimg.com/vi/WZ1hiSRmqiY/hqdefault.jpg")
                            end
                                imgui.NewLine()
                                if imgui.Checkbox(u8'Сокращенные команды', checkboxes.cmds) then cfg.settings.cmds = checkboxes.cmds.v save() end
				if imgui.IsItemHovered() then
                    imgui.BeginTooltip()
                        imgui.Text(u8'Нажми на кнопку "Скоращенные команды", чтобы узнать все команды')
                    imgui.EndTooltip()
                end
                                imgui.NewLine()
                                imgui.Text(u8'Специальные анимации')
                                imgui.TextQuestion(u8'0 - SPECIAL_ACTION_NONE \n1 - SPECIAL_ACTION_DUCK \n2 - SPECIAL_ACTION_USEJETPACK \n3 - SPECIAL_ACTION_ENTER_VEHICLE \n4 - SPECIAL_ACTION_EXIT_VEHICLE \n5 - SPECIAL_ACTION_DANCE1 \n6 - SPECIAL_ACTION_DANCE2 \n7 - SPECIAL_ACTION_DANCE3 \n8 - SPECIAL_ACTION_DANCE4 \n10 - SPECIAL_ACTION_HANDSUP \n11 - SPECIAL_ACTION_USECELLPHONE \n12 - SPECIAL_ACTION_SITTING \n13 - SPECIAL_ACTION_STOPUSECELLPHONE \n20 - SPECIAL_ACTION_DRINK_BEER \n21 - SPECIAL_ACTION_SMOKE_CIGGY \n22 - SPECIAL_ACTION_DRINK_WINE \n23 - SPECIAL_ACTION_DRINK_SPRUNK \n24 - SPECIAL_ACTION_CUFFED \n25 - SPECIAL_ACTION_CARRY \n68 - SPECIAL_ACTION_PISSING (cleo piss)')
                end
                        if menu == 11 then
				imgui.PushFont(fontsize)
        			imgui.CenterText(u8'URL')
        		imgui.PopFont()
				imgui.Separator()
				imgui.Text(u8'')
                                imgui.SetCursorPosY(30)
			    if imgui.Button(u8'YouTube', imgui.ImVec2(110, 20)) then
			       	os.execute("start https://youtube.com/")
			    end
                                imgui.SetCursorPosY(30)
                                imgui.SetCursorPosX(120)
                            if imgui.Button(u8'VK', imgui.ImVec2(110,20)) then
                                sampAddChatMessage('{FF0000}Отказано в доступе!')
                                --os.execute("start https://vk.com/audios545411710?section=all")
                            end
                               imgui.SetCursorPosY(60)
			    if imgui.Button(u8'Fa Icons', imgui.ImVec2(110, 20)) then
			       	os.execute("start https://fontawesome.com/icons?d=gallery&s=solid&m=free")
			    end
                   imgui.SetCursorPosY(60)
                   imgui.SetCursorPosX(120)
			    if imgui.Button(u8'Коды кнопок', imgui.ImVec2(110, 20)) then
			       	os.execute("start https://snipp.ru/handbk/vk-code")
			    end
                  end
                        if menu == 12 then
				imgui.PushFont(fontsize)
        			imgui.CenterText(u8'Code скриптов')
        		imgui.PopFont()
				imgui.Separator()
				imgui.Text(u8'Как использовать спец. анимации в разделе "Дополн\nительное"? \nДля начала нам нужно создать файл (название).lua \nэто можно сделать через конвентер файлов в инете \nзаходим в файл .lua (открываем в блокноте) и пишем')
                                imgui.Text(u8'Code')
                                imgui.TextQuestion(u8'require "lib.moonloader" --Загружаем библиотеки \n \nfunction main() --Функции скрипта \n    while not isSampAvailable() do wait(0) end \n    while true do \n        wait(0) \n        if wasKeyPressed(VK_XBUTTON1) and not sampIsCursorActive() then --VK_XBUTTON1 Кнопка на которую мы биндим анимку \n            sampSetSpecialAction(7) --(7) Номер анимки \n        end \n    end \nend')
                    imgui.SetCursorPosY(125)
                    imgui.SetCursorPosX(5)
                             if imgui.Button(u8'Скачать готовый lua script', imgui.ImVec2(165,20)) then
                                 os.execute("start https://drive.google.com/file/d/1-tZk_hYNtNR17_MHM5gGHxZb1BfKz6mK/view?usp=sharing")
                             end
                  end
                        if menu == 14 then
				imgui.PushFont(fontsize)
        			imgui.CenterText(u8'Язык')
        		imgui.PopFont()
				imgui.Separator()
                                imgui.SetCursorPosY(40)
                                imgui.SetCursorPosX(2)
			    if imgui.Button(u8'RU', imgui.ImVec2(320, 20)) then
                                sampAddChatMessage('{c0c0c0}Установлен {FFFFFF}Ру{0000FF}ск{FF0000}ий {c0c0c0}язык.', -1)
			    end
                   end
                        if menu == 15 then
				imgui.PushFont(fontsize)
        			imgui.CenterText(u8'Цены')
        		imgui.PopFont()
				imgui.Separator()
                                imgui.SetCursorPosY(40)
                                imgui.SetCursorPosX(2)
			...
			if menu == 9 then
        		...
        		imgui.PopFont()
				imgui.Separator()
				if imgui.Checkbox(u8'Pust Music', checkboxes.osplayer) then cfg.settings.osplayer = checkboxes.osplayer.v end
				imgui.TextQuestion(u8'Активация: /pustmusic\nЧтобы загрузить свои песни, откройте папку с игрой, \nдалее зайдите в moonloader/Pust Helper/Pust Music.')
    ...
    if frames.prmwindow.v then
    	...
    	imgui.Begin('PR Manager ('..thisScript().version..')##prmenu', frames.prmwindow, imgui.WindowFlags.NoResize)
	...
	if frames.colors.v then 
		imgui.SetNextWindowPos(imgui.ImVec2(resX / 2 , resY / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.SetNextWindowSize(imgui.ImVec2(500, 325), imgui.Cond.FirstUseEver)
		imgui.Begin('Colors Menu ('..thisScript().version..')', frames.colors, imgui.WindowFlags.NoResize)
...
function pustmusic()
	...
	imgui.Begin(u8'Pust Music by katanniy '..thisScript().version..'##music', frames.musicmenu, imgui.WindowFlags.NoResize)
...
function jobhelperimgui()
	if frames.bushelper.v then
        ...
        imgui.Begin('Bus Helper (v'..thisScript().version..')##bushelper', frames.bushelper, imgui.WindowFlags.NoResize)
    ...
    if frames.minehelper.v then
        ...
        imgui.Begin('Mine Helper (v'..thisScript().version..')##minehelper', frames.minehelper, imgui.WindowFlags.NoResize)
    ...
    if frames.farmhelper.v then
        ...
        imgui.Begin('Farm Helper (v'..thisScript().version..')##farmhelper', frames.farmhelper, imgui.WindowFlags.NoResize)
    ...
    if frames.fishhelper.v then
        imgui.SetNextWindowPos(imgui.ImVec2(resX / 2 , resY / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
        imgui.SetNextWindowSize(imgui.ImVec2(220, 115), imgui.Cond.FirstUseEver)
        imgui.Begin('Fish Helper (v'..thisScript().version..')##fishhelper', frames.fishhelper, imgui.WindowFlags.NoResize)
...
function msg(arg)
	sampAddChatMessage(color..'[Katanniy] {FFFFFF}'..textcolor..arg..'', -1)
end
...
--function sampev.onSetInterior(interior)
    --if interior == 80 then
        --msg('Interior: 80 (24/7)')
    --end
    --if interior == 18 then
        --msg('Interior: 18 (Центральный рынок)')
    --end
    --if interior == 71 then
        --msg('Interior: 71 (Трейлер)')
    --end
    --if interior == 2 then
        --msg('Interior: 2 (Паркинг)')
    --end
    --if interior == 0 then
        --msg('Interior: 0 (Улица)')
    --end
--end
...
function sampev.onShowDialog(id, style, title, button1, button0, text)
	...
	if checkboxes.ztimerstatus.v then
		if id == 0 and title:find('ПИЗДЕЦ!') then
				lua_thread.create(function() 
				msg('Тебе кинули зетку, GAZ GAZ GAZ!')
				ztimer = 550
					while ztimer > 0 do
